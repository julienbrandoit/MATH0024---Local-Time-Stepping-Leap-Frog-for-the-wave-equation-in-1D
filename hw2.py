...
def compare_time_evolution(problemSolver_as_a_function_of_dt, T, t_eval, dt_list, title):
    """
    TODO
    """

    fig, ax = plt.subplots(len(t_eval), len(dt_list), figsize=(16, 20), sharex=True, sharey=True)
    ...
    arrow_end_y = 0.075  # End of arrow in figure coordinates
...
if __name__ == "__main__":
    ...
    regular_mesh_problem = ClassicalFEM1DLeapFrog(n_elements_or_mesh=n_elements_coarse, dt=0.1, c=c, L=L, u0=lambda x: u0(x, sigma, L), v0=v0)
    refined_mesh_problem = ClassicalFEM1DLeapFrog(n_elements_or_mesh=refined_mesh, dt=0.1, c=c, L=L, u0=lambda x: u0(x, sigma, L), v0=v0)
    ...
    for ax_i, problem in zip(ax, [regular_mesh_problem, refined_mesh_problem]):
        ...
        for node in problem.fem.nodes:
            ax_i.plot(node.x, y_mesh, 'k', marker='d', markersize=5, label="Node position" if node.global_idx == 0 else None)
            ax_i.vlines(node.x, y_mesh, max(problem.u[node.global_idx], problem.u_previous[node.global_idx]), colors='black', linestyles='dashed', alpha=0.5, linewidth=1.5)
    ...
    ax[0].plot([node.x for node in regular_mesh_problem.fem.nodes], regular_mesh_problem.u, marker="o", markersize=5, linewidth=2, label=r"$\mathbf{u}_0$")
    ax[0].plot([node.x for node in regular_mesh_problem.fem.nodes], regular_mesh_problem.u_previous, marker="o", markersize=5, linewidth=2, label=r"$\mathbf{u}_{-1}$")

    ax[1].plot([node.x for node in refined_mesh_problem.fem.nodes], refined_mesh_problem.u, marker="o", markersize=5, linewidth=2, label=r"$\mathbf{u}_0$")
    ax[1].plot([node.x for node in refined_mesh_problem.fem.nodes], refined_mesh_problem.u_previous, marker="o", markersize=5, linewidth=2, label=r"$\mathbf{u}_{-1}$")
    ...
    # save the figure as initial_approximation.pdf
    ...
    ax.plot([n.x for n in regular_mesh_problem.fem.nodes], np.zeros(len(regular_mesh_problem.fem.nodes))+1, 'ro', markersize=10, alpha=0.5, label="Regular mesh")
    ax.plot([n.x for n in refined_mesh_problem.fem.nodes], np.zeros(len(refined_mesh_problem.fem.nodes)), 'bo', markersize=10, alpha=0.5, label="Refined mesh")
    ...
    ax.legend(fontsize=15, loc='upper center', bbox_to_anchor=(0.5, -0.5), ncol=2)
    plt.subplots_adjust(bottom=0.5)
    # save as mesh_pts1.pdf
    ...
    compare_time_evolution(regular_problemSolver_as_a_function_of_dt, T, t_eval, dt_list, title="regular_mesh_sol.pdf")
    ...
    compare_time_evolution(refined_problemSolver_as_a_function_of_dt, T, t_eval, dt_list, title="refined_mesh_sol.pdf")
